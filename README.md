...
#Bound   4.44s best:8     next:[1,7]      default_lp       ← lb 自己升到 1
#4       4.52s best:7     next:[1,6]                      ← 找到 cost=7
#Bound   4.80s best:7     next:[4,6]      max_lp           ← lb 升到 4
#Done    9.82s
```

- **`lb`（`next` 左值）**：**已证明**解的格数不可能低于它。由 LP 松弛 +
  约束传播**算出来**，随搜索逐步上升（上面 0 → 1 → 4）。
- **`ub`（`next` 右值）**：本次要找的解的格数**上限**——因为已找到的解
  再重复找到没有意义，所以只要严格更优的，即 `< best`。
- **`best`**：当前**已找到**的最好解的格数，是另一个独立的量。
  `best:19` 配 `next:[14,18]` 完全可能，含义就是你说的：
  **已证明 0–13 不可能，当前最好解是 19，接下来在 14–18 里找**。
- 当 `lb` 追上 `best`（如 `best:7` 且 `lb` 也到 7），**最优性得证**，直接 `#Done`。

**下界不是可以设定的参数，而是问题的计算结果。** 没有任何 API 能"把下界设成
13"：13 是否成立取决于 CP-SAT 自己的证明。而且知道"不可能低于 13"也**帮不上
找更优解**的忙——真正起作用的是**上界**和**一个现成的好解**，这正是 warm start
在做的事。
...
| 我们加什么 | 效果 |
| --- | --- |
| `obj <= C`（`--hint`） | 目标上界收到 C：不比上次差 |
| `obj <= C-1`（`--hint-strict`） | 目标上界收到 C-1：只找严格更优 |
