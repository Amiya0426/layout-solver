...
**为什么是压上界，而不是抬下界？** 这里有个容易写错的经典陷阱。
已知存在 cost = C 的解，于是"若存在严格更优解，则其 cost >= C+1"——
这句话本身没错，但它是**结论**，不能当**前提**用。如果把它写成硬约束
`obj >= C + 1`，就会和上界 `obj <= C` 同时成立，模型直接
`INFEASIBLE`，整轮搜索报废。只有当 C 确实不是最优时才碰巧不出问题，
一旦 C 已是最优（或历史解来自别的配置、并不满足当前模型）就必然自相矛盾。

所以这里**只压上界**，下界始终交给 CP-SAT 自己推理——日志里的
`next:[lb, ub]` 就是它自己算出的上下界，我们不去伪造 `lb`。
...
- 如果历史解偏陈旧（cost 比当前模型的真实最优还小），上界会偏紧，
  可能很快得到 `INFEASIBLE`；此时不要 `--hint-strict`，直接
  `--hint` 或干脆不带 `--hint` 重新搜索即可。

实测（`config.toy.json`）：

| 场景 | 命令 | 结果 |
| --- | --- | --- |
| 上次 cost=7（已最优） | `--hint --hint-strict` | `INFEASIBLE` ⇒ 免费得到"7 已最优"的证明 |
| 人为构造上次 cost=20 | `--hint --hint-strict` | 正常搜到并证明最优 cost=7 |
| 人为构造陈旧 hint cost=5 | `--hint`（旧代码会矛盾） | 正常搜到最优 cost=7 |
