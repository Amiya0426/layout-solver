...
- `--hint-strict`：配合 `--hint`，把目标下界也压到上次 cost+1（见下）；
...
目标函数是**整数**（传送带格数），而我们已经知道存在一个 cost = C 的可行解。
那么任何**严格更优**的解，其 cost 必然 `>= C + 1`。所以除了上界 `obj <= C`，
还能把**下界**压到 `C + 1`：
...
这等价于告诉求解器"只回报比上次更好的解"，可以把"和上次一样好"的那一大批
解整片剪掉，更快聚焦到改进解上。

代价与前提：

- 只有 Hint **完整**时才施加（每个可动模块都匹配到位姿、每条 net 两端都定位
  成功）；否则不能断定"那个 cost 的完整解存在于当前模型"，压下去可能误剪
  最优解，此时程序会打印 `Hint 不完整，已跳过下界收紧`；
- 如果上次的解**已经是全局最优**，模型会直接返回 `INFEASIBLE`——这不是错误，
  而是一个结论：**上次那个解就是最优**，程序会明确提示，且上一次的结果文件
  原样保留；
- `UNKNOWN` 则表示时间不够证明。

实测：`config.toy.json` 上次 cost=7（已最优），加 `--hint --hint-strict` 后
立刻返回 `INFEASIBLE`，等于是免费得到了"7 已最优"的证明。
